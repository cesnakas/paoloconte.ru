...
if ($zip->extractTo($_SERVER['DOCUMENT_ROOT'] . $tmpPath)) {
        ...
        foreach ($arFiles as $test) {
            ...
            if (!empty($test)) {
                ...
                if (Citfact\Paolo::recurseCopy($_SERVER['DOCUMENT_ROOT'] . $tmpPath . $test . '/photo/', $_SERVER['DOCUMENT_ROOT'] . $realPath . $test . '/photo/')) {
                    ...
                    if (Citfact\Paolo::setPicturesToElement($test, reset($mainImage), true)) {
    ...
    $shellAdapter = new \Bitrix\Scale\ShellAdapter();
    $shellAdapter->asyncExec("php -f $_SERVER[DOCUMENT_ROOT]/cron/activate_after_exchange.php");
} else {
