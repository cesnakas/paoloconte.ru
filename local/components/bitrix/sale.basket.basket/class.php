...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function getGridRows()
	{
		...
		if (!empty($this->basketItems))
		{
			...
			{
				$rows[$item['ID']] = $item;
                $rows[$item['ID']]['NAME'] = \Citfact\Tools::clearNameProd($rows[$item['ID']]['NAME']);
			}
		}
	...
	protected function checkCoupon($postList)
	{
		...
		if (!empty($postList['delete_coupon']))
		{
			...
			foreach ($postList['delete_coupon'] as $coupon)
			{
                ...
                \Citfact\CloudLoyalty\OperationManager::deletePromoCode($coupon);
			}
		}
		else
		{
			...
			if ($found)
			{
				if (!empty($coupon))
				{
                    ...
                    if(!$couponChanged) {
                        \Citfact\CloudLoyalty\OperationManager::addPromoCode($coupon);
                    }
				}
				else
				{
