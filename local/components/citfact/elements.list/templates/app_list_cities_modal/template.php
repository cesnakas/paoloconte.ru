...
<form action="#">
	<div class="modal-body">
		<div class="emulate-table city-box-1 full">
			<div class="emulate-row text valign-middle text-modal-city">
			...
			<ul>
				<?foreach($arResult['MAIN_CITIES'] as $arItem):?>
					<li>
						...
						<a href="/paoloconte_app/index.php?city=<?=$arItem['ID']?>" onclick="BXMobileApp.UI.Slider.setState(BXMobileApp.UI.Slider.state.CENTER); setTimeout(function() { BXMobileApp.UI.Page.reload(); }, 2000);"><?=$arItem['NAME']?></a>
					</li>
				<?endforeach?>
			</ul>
			<div class="clear"></div>
				...
				<div class="city-list <?=($key == $arResult['ACTIVE_LETTER']? 'active':'')?>" data-letter="<?=$key?>">
					<ul>
						...
						<?foreach($arItem as $key2=>$arCity):?>
							...
							<a href="/paoloconte_app/index.php?city=<?=$arCity['ID']?>" title="<?=$arCity['PROPERTY_OBLAST_VALUE'];?>" onclick="BXMobileApp.UI.Slider.setState(BXMobileApp.UI.Slider.state.CENTER); setTimeout(function() { BXMobileApp.UI.Page.reload(); }, 2000);">
							<li>
								<?=$arCity['NAME'];?>
							</li>
							</a>
