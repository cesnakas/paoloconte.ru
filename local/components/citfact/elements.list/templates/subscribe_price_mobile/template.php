...
if ($count_items == 0){
	...
	<div class="btn-box align-center">
		<a href="/catalog/" class="btn btn-green full big-2 fa mode1 fa-shopping-cart" style="width: 340px;"><span>Перейти в каталог товаров</span></a>
	</div>
<?
}
else{
				...
				<tr>
					...
					<td>
						<?if ($arResult['TOVARS'][$tovar_id]['STORE_AMOUNT'] > 0):?>
							<a href="<?=$tovar_url . '?id='.$arResult['TOVARS'][$tovar_id]['ID'] . '&action=ADD2BASKET&ajax_basket=Y'?>" class="btn-tobasket-insubscribe" data-product-id="<?=$arItem['ID']?>"><img src="/local/templates/paoloconte_mobile/images/svg/cart.svg"></a>
						<?else:?>
							Нет на складе
						<?endif;?>
					</td>
