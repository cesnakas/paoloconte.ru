...
{
    $cardNumber = str_replace('-','', $_REQUEST['UF_CARDNUMBER']);
    if(empty($cardNumber)){
        $cardNumber = Events::getCardIdForPhone($phone);
    }
	if ($_REQUEST['NAME'] == ''){
		$strError .= 'Имя обязательно для заполнения<br/>';
	}
	if ($_REQUEST['LAST_NAME'] == ''){
		$strError .= 'Фамилия обязательна для заполнения<br/>';
	}
    if (strpos(substr($_REQUEST['PERSONAL_BIRTHDAY'], -4), '_') == true) {
        $strError .= 'Неверная дата рождения.<br/>';
    }
	if ($_REQUEST['PERSONAL_PHONE'] == ''){
		$strError .= 'Телефон обязателен для заполнения<br/>';
	}
	if(!empty($cardNumber) && !Citfact\CloudLoyalty\OperationManager::checkCardNumberOfflineStore(str_replace('-','', $cardNumber))){
        $strError .= 'Некорректный номер карты<br/>';
    }

    if(!empty($cardNumber) && !Events::checkUserInCloudloyaltyCardAndPhone($cardNumber, $_REQUEST['PERSONAL_PHONE'])){
        $strError .= 'Карта '.$cardNumber . ' прикреплена к другому телефону<br/>';
    }

    if(!empty($cardNumber) && Events::checkCardUserInBitrix($cardNumber)){
        $strError .= 'Пользователь с такой картой уже зарегистрирован на сайте<br/>';
    }
	...
	if($strError == '')
	{
		...
		if(!empty($cardNumber)){
            $arFields['UF_LOYALTY_CARD'] = $cardNumber;
        }
