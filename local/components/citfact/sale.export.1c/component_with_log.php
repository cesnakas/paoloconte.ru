...
function getPrefixLastCommitedName() {
    $email = $GLOBALS['USER']->GetEmail();
    if ($email == '1c-ka_exchange@paoloconte.ru') {
        return 'ka_';
    }
...
function log1CMy123($message = '')
{
    try {
        $path = $_SERVER["DOCUMENT_ROOT"] . "/local/var/log1CMy123";
        mkdir($path, 0775, true);
        file_put_contents($path . "/log-" . date('Y-m-d') . ".log", date('H:i:s') . PHP_EOL, FILE_APPEND);
...
$curPage = substr($APPLICATION -> GetCurPage(), 0, 19);
...
else
{
	...
	elseif($_GET["mode"] == "query" || $_POST["mode"] == "query")
	{
		...
		if (!$bCrmMode)
		{
            ...
            if(strlen(COption::GetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_committed_".$curPage, ""))>0)
                $arFilter[">DATE_UPDATE"] = ConvertTimeStamp(COption::GetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_committed_".$curPage, ""), "FULL");

            COption::SetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_".$curPage, time());
		}
		else
		{
            ...
            Fact\CSaleExport::setLanguage('en');
		}
		...
		CTimeZone::Disable();
		$arResultStat = Fact\CSaleExport::ExportOrders2Xml(
        ...
        if (!$bCrmMode)
        {
            $time = intval($_SESSION["BX_CML2_EXPORT"][Fact\CSaleExport::getOrderPrefix()]);
            if($time>0)
                COption::SetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_".$curPage, $time);
        }
        else
	...
	{
        log1CMy123('mode success');
        log1CMy123('$_COOKIE');
        log1CMy123("export_session_name_".$curPage);
        log1CMy123($_COOKIE[COption::GetOptionString("sale", "export_session_name_".$curPage, "")]);
        log1CMy123(COption::GetOptionString("sale", "export_session_id_".$curPage, ""));
		...
		{
            $_SESSION["BX_CML2_EXPORT"][Fact\CSaleExport::getOrderPrefix()] = 0;

			COption::SetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_committed_".$curPage, COption::GetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_".$curPage, ""));

			log1CMy123('set time success');
			log1CMy123(getPrefixLastCommitedName() . "last_export_time_".$curPage);
			log1CMy123(COption::GetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_".$curPage, ""));
			log1CMy123(getPrefixLastCommitedName() . "last_export_time_committed_".$curPage);
			log1CMy123(COption::GetOptionString("sale", getPrefixLastCommitedName() . "last_export_time_committed_".$curPage, ""));
	...
	{
        log1CMy123('old version');
        try{
            if($ABS_FILE_NAME)
            {
                log1CMy123('$ABS_FILE_NAME');
                log1CMy123($ABS_FILE_NAME);
                ...
                if(isset($DATA) && $DATA !== false)
                {
                    ...
                    if($fp = fopen($ABS_FILE_NAME, "ab"))
                    {
                        ...
                        else
                        {
                            ...
                            log1CMy123('Ошибка записи в файл');
                            log1CMy123($FILE_NAME);
                        }
                        fclose($fp);
                    }
                    else
                    {
                        ...
                        log1CMy123('Ошибка открытия файла');
                        log1CMy123($FILE_NAME);
                    }
                }
                else
                {
                    ...
                    log1CMy123('Ошибка чтения HTTP данных');
                }
            }
            ...
            if(strlen($_SESSION["BX_CML2_EXPORT"]["zip"]) > 0)
            {
                ...
                if(function_exists("zip_open"))
                {
                    ...
                    log1CMy123('Распаковка на сайте невозможна. Отправьте незапакованный файл.');
                }
            }
            log1CMy123('zip');
            ...
            {
                log1CMy123('Fact\CSaleOrderLoader');
                $position = false;
                ...
                echo "success";
                log1CMy123('success old version');
                if(strlen($loader->strError)>0) {
                    echo $loader->strError;
                    log1CMy123('strError');
                    log1CMy123($loader->strError);
                }
                echo "\n";
            }
            else
            {
                ...
                log1CMy123('Файл для импорта пуст.');
            }

        } catch (\Exception $e) {
            log1CMy123('Exception old version');
            log1CMy123($e->getMessage());
        }
        log1CMy123('end old version');
	}
	elseif($_GET["mode"] == "file")// new version
	{
        log1CMy123('new version');
        try{
            if($ABS_FILE_NAME)
            {
                log1CMy123('$ABS_FILE_NAME');
                    ...
                    if($fp = fopen($ABS_FILE_NAME, "ab"))
                    {
                        ...
                        if($result === (function_exists("mb_strlen")? mb_strlen($DATA, 'latin1'): strlen($DATA)))
                        {
                            if($_SESSION["BX_CML2_EXPORT"]["zip"])
                                $_SESSION["BX_CML2_EXPORT"]["zip"] = $ABS_FILE_NAME;
                            echo "success\n";
                            log1CMy123('success new version');
                        }
                        else
                        {
                            echo "failure\n".GetMessage("CC_BSC1_ERROR_FILE_WRITE", array("#FILE_NAME#"=>$FILE_NAME));
            ...
            log1CMy123('Exception new version');
            log1CMy123($e->getMessage());
        }
        log1CMy123('end new version');
	}
	elseif($_GET["mode"] == "import" && $_SESSION["BX_CML2_EXPORT"]["zip"] && strlen($_SESSION["BX_CML2_EXPORT"]["zip"]) > 1)
	{
		...
		$result = Fact\CSaleExport::UnZip($_SESSION["BX_CML2_EXPORT"]["zip"], $_SESSION["BX_CML2_EXPORT"]["last_zip_entry"]);
		if($result===false)
		{
	...
	elseif($_GET["mode"] == "import" && $ABS_FILE_NAME)
	{
		...
		if(file_exists($ABS_FILE_NAME) && filesize($ABS_FILE_NAME)>0)
		{
			...
			else
			{
				...
				$_SESSION["BX_CML2_EXPORT"]["last_xml_entry"] = "";
				echo "success";
                log1CMy123("success import");
			}
			if(strlen($loader->strError)>0) {
                echo $loader->strError;
                log1CMy123("error import");
                log1CMy123($loader->strError);
            }
			echo "\n";
		}
	...
	{
        log1CMy123('Неизвестная команда');
