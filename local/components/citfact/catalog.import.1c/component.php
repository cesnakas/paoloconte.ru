...
elseif ($_GET["mode"]=="deactivate")
{
	if ($_GET["timestamp"] > 0)
	{
        ...
        //#61846# TASK START
        // Дополнение. Активация/деактивация товаров и торговых предложений из выгрузки в зависимости от их остатков и других условий
        // Константы IBLOCK_CATALOG, IBLOCK_SKU прописаны в /local/modules/citfact.tools/constants.php
        
        //DEBUG_LOG
        $log_file = "/home/bitrix/ext_www/paoloconte.ru/local/var/logs/1c_activate.log";
        $report = "CATALOG IMPORT START. TIMESTAMP:".$_GET["timestamp"]."\r\n";
        ...
        $arSelectProduct = array(
            ...
            "SECTION_ID",
            "XML_ID",
            "CATALOG_QUANTITY",         //количество на складе если нет ТП
            "PROPERTY_HAS_PHOTO",
        );
        ...
        $arNavStartParams = false;  //debug_param Если надо выводить не все элементы
        
        $rsProducts = CIBlockElement::GetList(array(), $arFilterProduct, false, $arNavStartParams, $arSelectProduct);
        $arProducts = array();
        while ($arField = $rsProducts->fetch()){
            $arProducts[$arField["XML_ID"]] = array(
                "ID" => $arField["ID"],
                "IBLOCK_ID" => $arField["IBLOCK_ID"],
                "ACTIVE_CURRENT" => $arField["ACTIVE"],
                "ACTIVE_REQUIRED" => "",
                "XML_ID" => $arField["XML_ID"],
                "CATALOG_QUANTITY" => $arField["CATALOG_QUANTITY"],                
                "HAS_PHOTO" => $arField["PROPERTY_HAS_PHOTO_VALUE"],
                "OFFERS_AMOUNT" => 0,
                "HAS_ACTIVE_OFFER" => "N"
            );
        }
        ...
        $arOffersID = array();
        $rsOffers = CIBlockElement::GetList(array(), $arFilterOffers, false, $arNavStartParams, $arSelectOffers);
        $arOffers = array();
        while ($arField = $rsOffers->fetch()){
            $arOffers[$arField["ID"]] = array(
                "ID" => $arField["ID"],
                "IBLOCK_ID" => $arField["IBLOCK_ID"],
                "XML_ID" => $arField["XML_ID"],
                "PARENT_XML_ID" => "",
                "ACTIVE_CURRENT" => $arField["ACTIVE"],
                "ACTIVE_REQUIRED" => "",
                "CATALOG_STORE_AMOUNT" => 0
            );
            $arOffersID[] = $arField["ID"];
        }
        
        //Получение количества товара для каждого из офферов
        $arSelectOffersCount = array(
            "PRODUCT_ID",
            "AMOUNT"
        );
        $arFilterOffersCount = array(
            "IBLOCK_ID" => IBLOCK_SKU,
            "STORE_ID" => 5 //Интернет-магазин Подольск
        );  
        if (count($arOffersID) < 500)
        {
            $arFilterOffersCount["PRODUCT_ID"] = $arOffersID;
        }

        $rsOffersCount = CCatalogStoreProduct::GetList(array(), $arFilterOffersCount, false, $arNavStartParams, $arSelectOffersCount);        
        
        $iter = 0;
        while ($arField = $rsOffersCount->fetch()){            
            if (!empty($arOffers[$arField["PRODUCT_ID"]]))
            {
                $arOffers[$arField["PRODUCT_ID"]]["CATALOG_STORE_AMOUNT"] = $arField["AMOUNT"];
                $iter++;
            }
        }  

        $arElementsUpdate = array(); //очередь элементов на обновление статуса
        $arProductsIdsUpdate = array(); //айди товаров для переиндексации
        
        //Обработка торговых предложений
        foreach ($arOffers as &$offer)
        {
            //добавляем XML_ID родительского товара
            $parentID = explode("#", $offer["XML_ID"]);
            $parentXmlId = "";
            if (count($parentID) == 2)
            {
                $offer["PARENT_XML_ID"] = $parentID[0];
                if (!empty($arProducts[$offer["PARENT_XML_ID"]]))
                {
                    $arProducts[$offer["PARENT_XML_ID"]]["OFFERS_AMOUNT"]++;
                }
            }
            //в зависимости от наличия на складах проставляется активность торгового предложения
            if ($offer["CATALOG_STORE_AMOUNT"] > 0)
            {
                $offer["ACTIVE_REQUIRED"] = "Y";
                $arProducts[$offer["PARENT_XML_ID"]]["HAS_ACTIVE_OFFER"] = "Y";
            }            
            else
            {
                $offer["ACTIVE_REQUIRED"] = "N";
            }    
            if ($offer["ACTIVE_CURRENT"] != $offer["ACTIVE_REQUIRED"])
            {
                if (intval($offer["ID"])>0)
                {
                    $arElementsUpdate[] = array(
                        "ID" => $offer["ID"],
                        "IBLOCK_ID" => $offer["IBLOCK_ID"],
                        "SET_ACTIVE" => $offer["ACTIVE_REQUIRED"]                
                    );                      
                }              
            }
        }
        ...
        {
            //Если у товара нет фото, деактивируем
            if ($product["HAS_PHOTO"] != "Y")
            {
                $product["ACTIVE_REQUIRED"] = "N";
            }
            else
            {
                //Проставляется активность товара в зависимости от доступности (наличие на складах + резервирование)
                    //если у товара нет предложений, сверяем количество на складах
                if ($product["OFFERS_AMOUNT"] == 0)
                {
                    if ($product["CATALOG_QUANTITY"] > 0)
                    {
                        $product["ACTIVE_REQUIRED"] = "Y";
                    }
                    else
                    {
                        $product["ACTIVE_REQUIRED"] = "N";
                ...
                {
                    $product["ACTIVE_REQUIRED"] = "N/A";
                    /* Отключена деактивация товара при деактивированных торговых предложениях
                    $product["ACTIVE_REQUIRED"] = "N";
                    if ($product["HAS_ACTIVE_OFFER"] == "Y")                 
                    {
                        $product["ACTIVE_REQUIRED"] = "Y";
                    }
                    */
            ...
            if (
                $product["ACTIVE_REQUIRED"] != "N/A" && 
                ($product["ACTIVE_CURRENT"] != $product["ACTIVE_REQUIRED"]) 
            )
            {
                if (intval($product["ID"])>0)
                {
                    $arProductsIdsUpdate[] = $product["ID"];
                    $arElementsUpdate[] = array(
                        "ID" => $product["ID"],
                        "IBLOCK_ID" => $product["IBLOCK_ID"],
                        "SET_ACTIVE" => $product["ACTIVE_REQUIRED"]                
                    );                       
                }             
            }            
        }
        ...
        //--Обновление элементов (товаров и торговых предложений)
        $element = new CIBlockElement;        
        $iterOffers = 0;
        $iterProducts = 0;

        $arIdsActivate = array();
        $arIdsDeactivate = array();
        
        foreach ($arElementsUpdate as $elementItem)
        {
            if ($elementItem["SET_ACTIVE"] == "Y")
            {
                $arIdsActivate[] = $elementItem["ID"];
            }
            else if ($elementItem["SET_ACTIVE"] == "N")
            {
                $arIdsDeactivate[] = $elementItem["ID"];
            }            
        }
        
        $queryActivate = "";
        if (count($arIdsActivate))
        {
            $idsActivate = implode(", ", $arIdsActivate);
            $queryActivate = "UPDATE b_iblock_element SET ACTIVE='Y' WHERE ID IN ($idsActivate);";
        }
        
        
        $queryDeactivate = "";
        if (count($arIdsDeactivate))
        {
            $idsDeactivate = implode(", ", $arIdsDeactivate);
            $queryDeactivate = "UPDATE b_iblock_element SET ACTIVE='N' WHERE ID IN ($idsDeactivate);";
        } 
        
        //DEBUG_LOG
        $report = "ACTIVATE:".$queryActivate."\r\n";
        $report .= "DEACTIVATE:".$queryDeactivate."\r\n";
        file_put_contents($log_file, $report, FILE_APPEND);          
        
        $connection = \Bitrix\Main\Application::getConnection();
        try 
        {
            if (strlen($queryActivate) > 0)
            {
                $connection->queryExecute($queryActivate);    
                file_put_contents($log_file, "ACTIVATION SUCCESS\r\n", FILE_APPEND);  //DEBUG_LOG                
            }            
        } 
        catch (\Exception $e) 
        {
            echo "Активация элементов прошла с ошибкой: ".$e;
            file_put_contents($log_file, "ACTIVATION FAILED\r\n", FILE_APPEND);  //DEBUG_LOG            
        }        

        try 
        {
            if (strlen($queryDeactivate) > 0)
            {
                $connection->queryExecute($queryDeactivate);    
                file_put_contents($log_file, "DEACTIVATION SUCCESS\r\n", FILE_APPEND);  //DEBUG_LOG                
            }  
        } 
        catch (\Exception $e) 
        {
            echo "Деактивация элементов прошла с ошибкой: ".$e;
            file_put_contents($log_file, "DEACTIVATION FAILED\r\n", FILE_APPEND);  //DEBUG_LOG            
        }  
        
        //--Запуск переиндексации фасеты для измененного товара
        foreach($arProductsIdsUpdate as $prodID)
        {
            \Bitrix\Iblock\PropertyIndex\Manager::updateElementIndex(IBLOCK_CATALOG, $prodID);
        }        
        file_put_contents($log_file, "FACETE INDEX UPDATED\r\n", FILE_APPEND);  //DEBUG_LOG        
		shell_exec("php -f $_SERVER[DOCUMENT_ROOT]/cron/activate_after_exchange.php");
        //#61846# TASK END
		...
		{
            // Если в настройках обмена не включена отмена на деактивацию элементов
            ...
            // Если в настройках обмена не включена отмена на деактивацию разделов
