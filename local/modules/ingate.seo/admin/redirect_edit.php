...
if ($request['ID'] > 0) {
	...
	$arData = RedirectTable::getList($filter)->fetch();
}
...
if ($isPost == "POST" && ($save != "" || $apply != "") && $RIGHT == "W" && check_bitrix_sessid()) {
	...
	if ($result->isSuccess()) {
		if (strlen($save) > 0) {
			LocalRedirect(INGATE_SEO_MODULE_ID."_redirects.php?lang=".LANGUAGE_ID);
		} else {
			LocalRedirect(
				INGATE_SEO_MODULE_ID."_redirect_edit.php?ID=".
...
$aMenu = array(
	array(
		...
		"LINK"	=> INGATE_SEO_MODULE_ID."_redirects.php?lang=".LANGUAGE_ID,
		"ICON"	=> "btn_list",
	)
);
...
?>
	...
	<tr class="adm-detail-required-field">
		<td><?=Loc::getMessage("INGATE_SEO_OLD")?></td>
		<td>
			<input type="text" name="OLD" value="<?=$arData['OLD'];?>" size="45">
		</td>
	</tr>
	<tr>
		<td><?=Loc::getMessage('INGATE_SEO_NEW')?></td>
		<td>
			<input type="text" name="NEW" size="120" value="<?=htmlspecialcharsbx($arData['NEW'])?>">
		</td>
	</tr>
	<tr>
		<td><?=Loc::getMessage('INGATE_SEO_REDIR_STATUS')?></td>
		<td>
			<select name="STATUS">
				<option <?=($arData['POSITION']=='301')?'selected=""':''?> value="301">301</option>
				<option <?=($arData['POSITION']=='302')?'selected=""':''?> value="302">302</option>
			</select>
		</td>
	</tr>
<?php
$tabControl->Buttons(array(
	"disabled" => ($RIGHT < "W"),
	"back_url" => INGATE_SEO_MODULE_ID."_redirects.php?lang=".LANGUAGE_ID,
));
$tabControl->End();
?>
</form>
