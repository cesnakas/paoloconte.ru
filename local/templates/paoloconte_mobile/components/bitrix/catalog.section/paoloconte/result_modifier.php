...
if (!empty($arResult['ITEMS']))
{
	...
	$arResult['SORT'] = array(
		...
		'DEFAULT' => array(
			'ACTIVE' => '',
			'URL' => $APPLICATION->GetCurPageParam("", $delParam),
		),
	);
	...
	$elementsCounter = array('20','60','120');
	foreach ($elementsCounter as $value) {
		$arResult['SHOW_ELEMENTS'][$value]['URL'] = $APPLICATION->GetCurPageParam("count=".$value, $delParam);
		$arResult['SHOW_ELEMENTS'][$value]['ACTIVE'] = (!empty($_REQUEST["count"]) && $_REQUEST["count"] == $value)? 'selected' : '';
	}
	...
	foreach ($arResult['ITEMS'] as &$arItem) {
		...
		if (!empty($arItem['OFFERS'])) {
            ...
            $new_price = $arItem['MIN_PRICE']['DISCOUNT_VALUE'];
		}
		else{
			if ($arItem['PRICES'][$price_name]['DISCOUNT_VALUE'] > 0
				&& $arItem['PRICES'][$price_name]['DISCOUNT_VALUE'] < $min_price
			) {
				$min_price = $arItem['PRICES'][$price_name]['DISCOUNT_VALUE'];
				$min_price_print = $arItem['PRICES'][$price_name]['PRINT_DISCOUNT_VALUE'];
			}
            $new_price =  $arItem['CATALOG_PRICE_'.$price_id];
		}

		//$oldPrice = ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $min_price) ? $min_price_print : '';
		//$newPrice = $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE'];
        $oldPrice = ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $min_price) ? $min_price : '';
		$newPrice = $new_price;
		...
		//echo "<pre style=\"display:block;\">"; print_r($arItem['SALE_PERCENT']); echo "</pre>";
	}
	unset ($arItem);
	...
	$count_str = '';
	if (strpos($_SERVER['REQUEST_URI'], '/search/' !== false) && $arResult['NAV_RESULT']->NavRecordCount != 0){
		$count_str = 'По вашему запросу нашли '.\Citfact\Tools::declension($arResult['NAV_RESULT']->NavRecordCount, array('элемент', 'элемента', 'элементов')).'.';
	}
	$arResult['COUNT_STR'] = $count_str;
