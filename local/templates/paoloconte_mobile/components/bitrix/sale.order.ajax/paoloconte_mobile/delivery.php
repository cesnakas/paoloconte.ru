...
<? $APPLICATION->AddHeadScript('https://points.boxberry.ru/js/boxberry.js');?>
<script>
	function pvz_delivery(result){
		document.getElementById('ORDER_PROP_7').value= 'Город: ' + result.name + "\n" + 'Пункт выдачи заказа: ' + result.address + "\n";
		return false;
	}
</script>
...
<div class="bx_section personal-box">
	<?
	if(!empty($arResult["DELIVERY"]))
	{
		...
		<div class="box-title"><?=GetMessage("SOA_TEMPL_DELIVERY")?></div>
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			if ($delivery_id !== 0 && intval($delivery_id) <= 0)
			{
				...
				{
					?>
					<div class="bx_block radio-box">
						<div class="bx_element line">
						...
						<div class="bx_element line">
							...
							<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?//=$clickHandler?>>
								...
								<div class="bx_description">
                                    ...
                                    <? // ссылка на выбор пункта самовывоза сама не появляется, если этот span не ссылка ?>
                                    <? if (in_array($delivery_id, array(SDEK_PROFILE_PICKUP, SDEK_PROFILE_PICKUP_ONLINE)) && $arDelivery["CHECKED"] == "Y") { ?>
                                        <div id="choose_pickup"></div>
                                    <? } ?>
