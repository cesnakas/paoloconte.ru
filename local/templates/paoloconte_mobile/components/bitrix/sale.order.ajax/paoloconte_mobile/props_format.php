			...
			?>
				<div>
					...
					foreach ($arSource as $arProperties)
					{
						...
						?>
						<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" class="<?= ($hidden) ? 'hidden':''?>">
...
{
	function PrintLocationAndAddress($arSource = array(), $locationTemplate = ".default", $saved_location, $saved_address = '', $arError = array())
	{
		if (!empty($arSource))
		{
			?>
				...
				foreach ($arSource as $arProperties)
				{
					...
					<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						if ($arProperties["TYPE"] == "TEXTAREA")
						{
                            $arErroeBorderColor = '';
                            if(isset($arError['PRINT'][$arProperties['ID']])) {
                                $arErroeBorderColor = 'border-color: red';
                            }
							$rows = ($arProperties["SIZE2"] > 10) ? 4 : $arProperties["SIZE2"];
							?>
							<br/>
