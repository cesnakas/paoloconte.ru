...
<div class="detail-wrap clear-after" id="<? echo $arItemIDs['ID']; ?>">
                ...
                <? foreach ($arResult['CATALOG_IMG']['PHOTO'] as $photo) { ?>
                    <li class="emulate-table full">
                        <div class="emulate-cell valign-middle image image-cont">
                            <a class="fancybox-detail" rel="gallery_1" href="<?= $photo['BIGGEST'] ?>"
    ...
    <div class="align-center">
        <div class="action green">
            <a href="#" data-type="getMovedPanel" data-target="#side-low-price">Узнать о снижении цены</a>
        </div>
    </div>
    ...
    <div class="bottom-options">
                ...
                <div class="color-box">
                    <? foreach ($arResult['OTHER_COLORS'] as $arColor): ?>
                        ...
                        <a href="<? if ($active): ?>javascript:void(0)<? else: print $arColor['DETAIL_PAGE_URL']; endif ?>"
                           ...
                           style="background-image: url('<?= $arColor['COLOR']['FILE_PATH'] ?><? //=$arColor['IMAGES']['PHOTO'][0]['SMALL']?>')"></a>
                    <? endforeach ?>
                </div>
            </div>
        <? endif ?>
        ...
        <div class="btn-box">
            ...
            <? if ($arResult['PROPERTIES']['OFFERS_AMOUNT']['VALUE'] > 0) { ?>
                <a href="#" onmousedown="try { rrApi.addToBasket(<?= $arResult['ID'] ?>) } catch(e) {}"
                   class="btn btn-gold full btn-tobasket"
                   data-product-id="<?= $productId ?>"><span>Добавить в корзину</span></a>
                <div class="info-tobasket">Товар добавлен в корзину</div>
                <a href="#" onmousedown="try { rrApi.addToBasket(<?= $arResult['ID'] ?>) } catch(e) {}"
                   class="btn btn-gray-dark full fast-order" data-product-id="<?= $productId ?>"
                   data-type="getMovedPanel" data-target="#side-fast-order"><span>Быстрый заказ</span></a>
                <br/>
            <? } else { ?>
            ...
            <div class="align-center">
            	<span class="to-favorite" data-type="getMovedPanel" data-target="#side-to-favorite"
                      data-product-id="<?= $productIdToFav ?>" data-image="<?= $productPhoto ?>">
    ...
    <div class="panel-group rotate styled-detail-property" id="" role="tablist" aria-multiselectable="true">
        <div class="panel">
            <div class="panel-heading pts-bold" role="tab" id="heading-f1">
                <a class="collapsed clear-after" data-toggle="collapse" href="#collapse-f1" aria-expanded="true"
                   aria-controls="collapse-f1">
        ...
        <div class="panel">
            <div class="panel-heading pts-bold" role="tab" id="heading-f2">
                <a class="collapsed clear-after" data-toggle="collapse" href="#collapse-f2" aria-expanded="true"
                                        ...
                                        <div class="action-modal-wrap hover-mod">
                                            <div class="shop-name get-modal">
                                                ...
                                                <ul>
                                                    ...
                                                    <? if ($graphick != ''): ?>
                                                        <li><i class="fa fa-clock-o"></i> <?= $graphick ?>
                                                        </li><? endif; ?>
                                                    <? if ($phone != ''): ?>
                                                        <li><i class="fa fa-phone"></i> <?= $phone ?></li><? endif; ?>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="panel">
            <div class="panel-heading pts-bold" role="tab" id="heading-f3">
                <a class="collapsed clear-after" data-toggle="collapse" href="#collapse-f3" aria-expanded="true"
                   aria-controls="collapse-f3">
        ...
        <div class="panel">
            <div class="panel-heading pts-bold" role="tab" id="heading-f4">
                <a class="collapsed clear-after" data-toggle="collapse" href="#collapse-f4" aria-expanded="true"
                   aria-controls="collapse-f4">
                    <span class="head-text">Размерная сетка</span>
                    <i class="fa fa-caret-down"></i>
                </a>
            </div>
            <div id="collapse-f4" class="panel-collapse collapse tab_table_size" role="tabpanel"
                 aria-labelledby="heading-f4">
                <div class="panel-body">
    ...
    <div class="container">
        <div data-retailrocket-markup-block="5694ac169872e53858b742bf" data-product-id="<?= $arResult['ID'] ?>"></div>
    </div>
    <? if (!empty($arResult['OTHER_MODELS']) and false): ?>
                                ...
                                <a href="<?= $arModel['DETAIL_PAGE_URL'] ?>">
                                    <div class="image">
                                        <? if (!empty($arModel['IMAGES']['PHOTO'][0]['SMALL'])): ?>
                                            <img src="<?= $arModel['IMAGES']['PHOTO'][0]['SMALL'] ?>"
                                                 alt="<?= $arModel['NAME'] ?>">
                                        <? else: ?>
                                            <img src="<?= $arResult['NOPHOTO'] ?>" alt="нет фото">
                                        <? endif ?>
...
<div class="reviews-wrap">
    ...
    <a href="#" class="get-review btn btn-gray-dark full big" data-type="getMovedPanel" data-target="#side-get-review">
        <span>Оставить отзыв</span>
    </a>
</div>

<? $this->SetViewTarget("detail_add_review_block"); ?>
<nav id="side-get-review" class="moved-panel side-get-review">
...
<nav id="side-fast-order" class="moved-panel side-low-price">
    <div class="moved-panel-wrap">
        <div class="moved-panel-head" class="">
            <h6>Быстрый заказ</h6>
        ...
        <div class="moved-panel-body">
            <div class="aside-box fast-order-wrap">
                <? $APPLICATION->IncludeComponent("citfact:form.ajax", "fast_order_mobile", Array(
                    ...
                    "SUCCESS_MESSAGE" => "Ваша заказ принят. Мы перезвоним вам в ближайшее время.",
                    "ELEMENT_ACTIVE" => "Y",
...
<nav id="side-low-price" class="moved-panel side-low-price">
    <div class="moved-panel-wrap">
        <div class="moved-panel-head" class="">
...
<script type="text/javascript">
    ...
    $(function () {
        $(".item.alt.iconNext").on("click", function () {
            console.log($(this).attr('href'));
            location.href = $(this).attr('href') + '?t=' + $(window).scrollTop();
            return false;
        });
        ...
        var scrollTime = 1;
        $("body,html").animate({"scrollTop":<?=$_GET['t'];?>}, scrollTime);
        <?endif;?>
    });
</script>
