	...
	<table class="full">
		...
		foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
			...
			<tr>
				...
				<td>
					<div class="index"><?=$arItem['ARTICUL']?></div>
					<div class="name">
						<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="/paoloconte_app<?=$arItem["DETAIL_PAGE_URL"] ?>"><?endif;?>
							<?=$arItem["NAME"]?>
						<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
					</div>
					<?if ($arItem["RAZMER"] != ''): ?>
						<div class="size">Размер: <?=$arItem["RAZMER"]?></div>
					<?endif?>
				</td>
				<td>
					<div class="price-box">
						<?// TODO: вывод старой цены (не приходит в $arItem) ?>
				...
				<td>
					<div class="status">
						<?//if ($arItem['OFFERS_AMOUNT'] > 0):?>
						<?if ($arResult['STORE_AMOUNT'][ $arItem['PRODUCT_ID'] ] > 0):?>
							<span class="round-label true"><i class="fa fa-check"></i></span>
						<?else:?>
							<span class="round-label false"><i class="fa fa-times"></i></span>
						<?endif;?>
					</div>
				</td>
				<td>
					<?/*<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?>" class="btn full btn-gray big fa mode1 fa-shopping-cart"><span>Добавить в корзину</span></a>*/?>
					<?//if ($arItem['OFFERS_AMOUNT'] > 0):?>
					<?if ($arResult['STORE_AMOUNT'][ $arItem['PRODUCT_ID'] ] > 0):?>
						<?$tovar_url = $arItem ['DETAIL_PAGE_URL'];?>
						<a href="/paoloconte_app<?=$tovar_url.'?id='.$arItem['PRODUCT_ID'].'&action=ADD2BASKET&ajax_basket=Y'?>" class="btn-tobasket-infavorites" data-product-id="<?=$arItem['PRODUCT_ID']?>"><img src="/local/templates/paoloconte_mobile/images/svg/cart.svg"></a>
					<?endif;?>
				</td>
