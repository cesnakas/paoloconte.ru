...
$res = CIBlockElement::GetList(array('ID' => 'ASC'), $arFilter, false, false, array("ID", "IBLOCK_ID", "NAME", "DATE_CREATE", "PROPERTY_USER_ID", "PROPERTY_USER_NAME", "PROPERTY_MESSAGE", "PROPERTY_STARS"));
while ($ob = $res->GetNext(false, false)) {
	$arTemp = array();
	$username = '';
	if ($ob['PROPERTY_USER_ID_VALUE'] != '') {
		...
		$arTemp['USERNAME'] = $arUser['NAME'].' '.substr($arUser['LAST_NAME'], 0, 1).'.';
...
if(!empty($arResult['OFFERS'])) {
	...
	foreach ($arResult['OFFERS'] as $key => $arOffer) {
		if ($arOffer['PRICES'][$price_name]['DISCOUNT_VALUE'] > 0 && $arOffer['PRICES'][$price_name]['DISCOUNT_VALUE'] < $oldPrice) {
			$oldPrice = $arOffer['PRICES'][$price_name]['DISCOUNT_VALUE'];
			$oldPrice_print = $arOffer['PRICES'][$price_name]['PRINT_DISCOUNT_VALUE'];
		}
		if ($arOffer['PRICES'][$price_action_name]['DISCOUNT_VALUE'] > 0 && $arOffer['PRICES'][$price_action_name]['VALUE'] < $newPrice) {
...
$arAmount = array();
$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' =>$arIdsOffers, 'STORE_ID' => $arIdsStores), false, false, array());
