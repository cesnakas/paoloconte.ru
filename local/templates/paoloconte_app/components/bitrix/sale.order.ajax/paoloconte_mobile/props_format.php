...
{
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default", $arUser = array())
	{
		global $USER;
			...
			?>
				<div>
					...
					foreach ($arSource as $arProperties)
					{
						...
						?>
						<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" class="<?=$arProperties['CODE'] == 'ZIP' ? 'hidden':''?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							...
							<div class="bx_block r3x1">
								...
								?>
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$value?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" class="<?=$class;?>" />
...
{
	function PrintLocationAndAddress($arSource = array(), $locationTemplate = ".default", $saved_location, $saved_address = '')
	{
		if (!empty($arSource))
		{
			?>
				...
				foreach ($arSource as $arProperties)
				{
					...
					<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						if ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							<div class="bx_block r3x1">
								<textarea rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=($saved_address!=''? $saved_address:$arProperties["VALUE"])?></textarea>
