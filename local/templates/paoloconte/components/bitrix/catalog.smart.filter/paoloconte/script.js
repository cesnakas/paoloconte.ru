...
$('form .send-form').on('click', function () {
	JCSmartFilter.prototype.sendFormFilter(this);
});

JCSmartFilter.prototype.sendFormFilter = function(input)
{
	var values = [];
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			else
			{
				...
				// Hide founded count tip
				clearTimeout(this.timer_modef);
				this.timer_modef = setTimeout( function(){$('#modef').fadeOut(); }, 7000);
			}
		}
	}
};
...
$(document).on("click",'.selected-fitler [data-id]',function(){
	var id=$(this).data("id");
	$('#'+id).click();
	$(this).hide();
});