...
?>
			...
			<td class="forum-cell-post">
				...
				if ($arRes["USER"]["PERMISSION"] >= "Q" && $res["SHOW_CONTROL"] != "N"):
					?>&nbsp;
					<div class="check-box check-box-size3" onclick="this.firstElementChild.click();">
						<input type="checkbox" name="message_id[]" value="<?=$res["ID"]?>" id="message_id_<?=$res["ID"]?>_" <?
						if ($res["CHECKED"] == "Y"):
						?> checked="checked" <?
						endif;
							?> onclick="SelectPost(this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode)" />
