...
<?
						if ($arParams["SHOW_AUTHOR_COLUMN"] == "Y"):
?>
					<td class="forum-column-author <?if ($result==0) echo 'forum-raw-background-grey';?>"><span><?
...
?>
					<td class="forum-column-replies forum-cell-hidden <?if ($result==0) echo 'forum-raw-background-grey';?>"><span><?=$res["POSTS"]?> <?
...
?>
					<td class="forum-column-views <?if ($result==0) echo 'forum-raw-background-grey';?>">
						<span><?=$res["VIEWS"]?></span>
						<p><?echo \Citfact\Tools::declension(intVal($res["VIEWS"]), array('просмотр','просмотра','просмотров'),true);?></p>
					</td>
					<td class="forum-column-lastpost <?if ($result==0) echo 'forum-raw-background-grey';?>"><?
						if ($arResult["PERMISSION"] >= "Q"):
?>
						<div class="forum-select-box check-box" onclick="this.firstElementChild.click();">
							<input type="checkbox" name="TID[]" value="<?=$res["ID"]?>" onclick="SelectRow(this.parentNode.parentNode.parentNode)" />
...
?>
						<div class="forum-lastpost-box">
							<span class="forum-lastpost-from">
								<noindex><?=GetMessage("F_LAST_FROM")?>&nbsp;<span class="forum-lastpost-orange-name"><?=$res["LAST_POSTER_NAME"]?></span></noindex>
							</span>
