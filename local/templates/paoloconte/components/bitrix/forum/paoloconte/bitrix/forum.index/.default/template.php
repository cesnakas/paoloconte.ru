...
?>
			<thead>
				<tr>
					<th class="forum-column-title" colspan="5"><div class="forum-head-title"><span><?=GetMessage("F_FORUM_NAME")?></span></div></th>
...
?>
		<thead>
			<tr>
				<th class="forum-column-title" colspan="4" scope="col"><div class="forum-head-title"><span><?
					?><noindex><a rel="nofollow" href="<?=$arResult["URL"]["GROUP_".$arGroup["ID"]]?>"><?
					?><?=$arGroup["NAME"]?></a></noindex></span></div></th>

				<th class="forum-column-lastpost" scope="col">
					<div class="forum-column-show-hide" onclick='hideButton("#forum-tbody-<?=$arGroup["ID"]?>","#forum-hide-show-button-<?=$arGroup["ID"]?>")'>
						<span id="forum-hide-show-button-<?=$arGroup["ID"]?>">__</span>
					</div>
				</th>
...
<?
		...
		{
?>
			<tbody id="forum-tbody-<?echo $arGroup["ID"]?>">
<?
			foreach ($arGroup["FORUMS"] as $key=>$res)
			{
				$result = $key % 2; 
...
?>
			<tr class="<?=($iCountRows == 1 ? "forum-row-first " : "")?><?
				...
				?>>
					<td class="forum-column-icon <?if ($result==0) echo 'forum-raw-background-grey';?>">
...
?>
					...
					<td class="forum-column-title <?if ($result==0) echo 'forum-raw-background-grey';?>">
...
?>
					...
					<td class="forum-column-topics <?if ($result==0) echo 'forum-raw-background-grey';?>">
						<span><?=$res["TOPICS"]?></span>
						<p><?echo \Citfact\Tools::declension(intVal($res["TOPICS"]), array('тема','темы','тем'),true);?></p>
					</td>
					<td class="forum-column-replies <?if ($result==0) echo 'forum-raw-background-grey';?>">
						<span><?=$res["POSTS"]?></span>
						<p><?echo \Citfact\Tools::declension(intVal($res["POSTS"]), array('ответ','ответа','ответов'),true);?></p>
					</td>
					<td class="forum-column-lastpost <?if ($result==0) echo 'forum-raw-background-grey';?>">
<?
					if (intVal($res["LAST_MESSAGE_ID"]) > 0):
?>
						<div class="forum-lastpost-box">
							<span class="forum-lastpost-title">
								<noindex><?=GetMessage("F_LAST")?>&nbsp;<a rel="nofollow" href="<?=$res["URL"]["MESSAGE"]?>" title="<?=htmlspecialcharsEx($res["~TITLE"]." (".$res["~LAST_POSTER_NAME"].")")?>"><span class="forum-lastpost-orange-title"><?=$res["TITLE"]?></span></a></noindex>
							</span>
							<span class="forum-lastpost-from">
								<noindex><?=GetMessage("F_FROM")?>&nbsp;<span class="forum-lastpost-orange-name"><?=$res["LAST_POSTER_NAME"]?></span></noindex>
							</span>
...
?>
					...
					<td class="forum-column-topics"><span><?=$res["TOPICS"]?>&nbsp;<?=GetMessage("F_TOPICS")?></span></td>
					<td class="forum-column-replies"><span><?=$res["POSTS"]?>&nbsp;<?=GetMessage("F_ANSWERS")?></span></td>
