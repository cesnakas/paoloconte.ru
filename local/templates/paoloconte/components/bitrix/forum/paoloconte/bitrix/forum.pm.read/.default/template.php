...
?>	
		<div class="forum-pm-notification">
			...
			<form action="<?=$APPLICATION->GetCurPageParam()?>" method="get" name="PMESSAGE" class="forum-form" >
				...
				<?=bitrix_sessid_post()?>
				<div class="btn ok2 btn-green mode2 icon-arrow-right">
				<input type="submit" class="forum-mess-button ok-btn2" value="<?=GetMessage("PM_SEND_NOTIF")?>" />
				</div>
			</form>
		</div>
<?
...
<?*/?>
		...
		<form class="forum-form" action="<?=POST_FORM_ACTION_URI?>" method="POST">
			...
			<?=bitrix_sessid_post()?>
			<span class="forum-white-text"><?=GetMessage("PM_ACT_MOVE")?> <?=GetMessage("PM_IN")?>:&nbsp;</span>
			<select name="folder_id">
			<?
			foreach ($arResult["FOLDERS"] as $res)
			{
