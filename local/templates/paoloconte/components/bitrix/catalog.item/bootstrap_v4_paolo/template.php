...
$priceModal = new \Citfact\Entity\Price\PriceModal();
foreach ($arResult['ITEM']['PRICES'] as $v_price) {
    if ($v_price['PRICE_ID'] == Paolo::getBasePriceId()) {
        $priceModal->basePrice = $v_price['DISCOUNT_VALUE'];
    } else if ($v_price['PRICE_ID'] == Paolo::getActionPriceId()) {
        if (isset($v_price['DISCOUNT_VALUE'])) {
            $priceModal->setIsDiscount(true);
            $priceModal->actionPrice = $v_price['DISCOUNT_VALUE'];
        }
    }
}
...
if (isset($arResult['ITEM']))
{
    ...
    $price = $priceModal->updatePriceForTemplate($price);
