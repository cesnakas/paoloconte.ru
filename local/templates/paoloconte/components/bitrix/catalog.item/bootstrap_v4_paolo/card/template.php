...
$productTitle = $item['PROPERTIES']['NAIMENOVANIE_MARKETING']['VALUE'];
...
<div class="product-item" data-id="<?=$item['ID']?>">
	...
	<? endif; ?>
        ...
        $articul = trim($item['PROPERTIES']['CML2_ARTICLE']['VALUE']);
        $productImage  = Citfact\Paolo::getProductImage($articul, $imageConfig);
        if ($productImage) {
            $bgImage = $productImage['PHOTO'][0]['SMALL'];
        } else {
            $bgImage = Core::NO_PHOTO;
        }
        ?>
        <span style="display: none;" id="<?=$itemIds['PICT']?>"></span>
		<span class="product-item-image-original" style="background-image: url('<?=$bgImage?>'); <?=($showSlider ? 'display: none;' : '')?>"></span>
		<?

		if ($item['SECOND_PICT'])
		{
			?>
            <span style="display: none;" id="<?=$itemIds['SECOND_PICT']?>"></span>
			<span class="product-item-image-alternative" style="background-image: url('<?=$bgImage?>'); <?=($showSlider ? 'display: none;' : '')?>"></span>
			<?
		}
...
<? endif; ?>
			...
			switch ($blockName)
			{
				case 'price': ?>
					<div class="product-item-info-container product-item-price-container" data-entity="price-block">
						<?
						if ($arParams['SHOW_OLD_PRICE'] === 'Y' || $priceModal->isDiscount())
						{
							?>
						...
						<span class="product-item-price-current">
                            <span id="<?=$itemIds['PRICE']?>" style="display: none;"></span>
							<?
							if (!empty($price))
							{
                        ...
                        <? if ($priceModal->isDiscount()) { ?>
                            <div class="c-g-item-discount">
                                <span class="tag__item tag__item--sell"><?=$priceModal->getDiscountPercent()?>%</span>
                            </div>
                        <? } ?>
					</div>
					<?
				...
				case 'sku':
					if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y' && $haveOffers && !empty($item['OFFERS_PROP']))
					{
						...
						<div class="product-item-info-container product-item-hidden" id="<?=$itemIds['PROP_DIV']?>">
							...
							{
							    if ($skuProperty['CODE'] == 'RAZMER') {
                                    $skuProperty['NAME'] = 'Размеры';
                                }
	...
	?>
    <?
    $colors = Core::getInstance()->getAllColors();
    $color = $colors[$item['PROPERTIES']['TSVET_DLYA_FILTRA']['VALUE']];
    $paramsForModal = [
        'name' => $productTitle,
    ...
    ?>
    <div class="data-for-modal" data-id="<?=$item['ID']?>" style="display: none"
         data-params="<?=htmlspecialchars(json_encode($paramsForModal))?>"></div>
</div>
