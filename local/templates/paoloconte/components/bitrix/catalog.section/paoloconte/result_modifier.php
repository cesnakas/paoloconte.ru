...
if (!empty($arResult['ITEMS']))
{
	...
	{
        $arItem['PROPERTY_NAIMENOVANIE_MARKETING_VALUE'] = $arItem['PROPERTIES']['NAIMENOVANIE_MARKETING']['VALUE'];
	...
	$arResult['SORT'] = array(
		'PRICE' => array(
			'ACTIVE' => '',
			'URL' => $APPLICATION->GetCurPageParam("by=PROPERTY_PRICE_PROP&amp;order=asc", $delParam),
		),
		'DISCOUNT' => array(
			'ACTIVE' => '',
	...
	if(!empty($_REQUEST["by"]) && !empty($_REQUEST["order"])){
		switch ($_REQUEST["by"]){
			...
			case 'PRICE':
			case 'PROPERTY_PRICE_PROP':
				if($_REQUEST["order"] == "asc") {
					//$arResult['SORT']['PRICE']['URL'] = $APPLICATION->GetCurPageParam("by=".$arResult['PRICE_CODE']."&amp;order=desc", $delParam);
					$arResult['SORT']['PRICE']['URL'] = $APPLICATION->GetCurPageParam("by=PROPERTY_PRICE_PROP&amp;order=desc", $delParam);
				...
				elseif($_REQUEST["order"] == "desc") {
					...
					$arResult['SORT']['PRICE']['URL'] = $APPLICATION->GetCurPageParam("by=PROPERTY_PRICE_PROP&amp;order=asc", $delParam);
					$arResult['SORT']['PRICE']['ACTIVE'] = "active";
				}
				break;
			case 'DISCOUNT':
...
*/
	...
	foreach ($arResult['ITEMS'] as &$arItem) {
		...
		//$min_price = (defined('PHP_INT_MAX')) ? 0 : 9999999;
		$min_price_print = '';
    ...
    // Один раз выполним запрос на получение всех Offers
    $resItems = [];
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        $resItems[] = $arItem["ID"];
    }
    $res = \CCatalogSKU::getOffersList(
        $resItems, // массив ID товаров
        $iblockID = 0, // указываете ID инфоблока только в том случае, когда ВЕСЬ массив товаров из одного инфоблока и он известен
        $skuFilter = array($arrFilter["OFFERS"]), // дополнительный фильтр предложений. по умолчанию пуст.
    ...
    foreach ($res as $offerKey => $offerValue) {
        foreach ($offerValue as $offerKey2 => $offerValue2) {
            $arIdsOffersCheck[$offerKey][] = $offerValue2["ID"];
        }
    }

    $sectionIdPage = '';
    $arFilter = Array('IBLOCK_ID' => IBLOCK_CATALOG, 'CODE' => $arParams['SECTION_CODE']);
    $db_list = \CIBlockSection::GetList(Array(), $arFilter, false, Array('ID', 'IBLOCK_SECTION_ID'), false);
    if ($arSect = $db_list->GetNext()) {
        $sectionIdPage = $arSect['ID'];
        $sectionId = $arSect['IBLOCK_SECTION_ID'];
    }

    $productAvailabilityService = new ProductAvailability();
    $sectionsCond = $productAvailabilityService->getSectionsInfoByType();

    $keyConditionString = '';
    foreach ($sectionsCond['CONDITIONS'] as $key => $value) {
        if (in_array($sectionId, $value)) {
            $keyConditionString = $key;
        }
    }
    $productAvailabilityBuy = new ProductAvailabilityBuy();
    $STORE_ID = [5];

    //получаем все группы элемента
    $productsIds = array_column($arResult['ITEMS'], 'ID');
    $db_old_groups = CIBlockElement::GetElementGroups($productsIds, true);
    while ($ar_group = $db_old_groups->Fetch()) {
        if($productAvailabilityBuy->isSectionChildClothes($ar_group["ID"])){
            $STORE_ID[]=80;
            break;
        } 
    }
     ...
     foreach ($arResult['ITEMS'] as $key => $arItem) {
		if (!empty($arItem['OFFERS'])) {
            ...
            $arResult['OFFERS_CHECK'] = "N";
			while ($arStore = $rsStore->Fetch()){
			    if(empty($arResult['OFFERS_AMOUNT'] [$arStore['PRODUCT_ID']])){
                    $arResult['OFFERS_AMOUNT'] [$arStore['PRODUCT_ID']] = $arStore['AMOUNT'];
                }else{
                    $arResult['OFFERS_AMOUNT'] [$arStore['PRODUCT_ID']] += $arStore['AMOUNT'];
                }
                if ((in_array($arStore['PRODUCT_ID'], $arIdsOffersCheck[$arItem["ID"]])) && ($arStore['AMOUNT'] > 0)) {
                    $arResult['OFFERS_CHECK'] = "Y";
                }
			}

			if (in_array('LEFTOVERS', $sectionsCond['COND_VALUES'][$keyConditionString])) {
                ...
                if ((count($arResult['ITEMS'][$key]['OFFERS']) == 0) || ($arResult['OFFERS_CHECK'] == "N")) {
...
//                    }
    ...
    if (!$arParams["ADD_SECTIONS_CHAIN"]) {
        ...
        $this->__component->arResultCacheKeys = array_merge($this->__component->arResultCacheKeys, array('PATH', 'SECTION_PAGE_URL','IBLOCK_ID'));
    }
...
if (!empty($arResult["UF_IMAGE"])) {
    $file = CFile::ResizeImageGet($arResult["UF_IMAGE"], array('width' => 750, 'height' => 400), BX_RESIZE_IMAGE_EXACT, true);
    $arResult["UF_IMAGE"] = $file['src'];
}
if (empty($arResult["UF_BANNER_LINK"])) {
    $arResult["UF_BANNER_LINK"] = "javascript:void(0);";
}

$this->__component->setResultCacheKeys(['COUNT_ITEMS']);
