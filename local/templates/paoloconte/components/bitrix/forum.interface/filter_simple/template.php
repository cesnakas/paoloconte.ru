...
?>
	...
	<div class="forum-filter-field <?=$res["CLASS"]?>">
		<label class="forum-filter-field-title name-cell2" for="<?=$res["ID"]?>"><?=$res["TITLE"]?>:</label>
...
?>
				<option value="<?=$key?>" <?=($res["ACTIVE"] == $key ? " selected='selected'" : "")?>><?=trim(str_replace(
...
?>
		<div class="check-box">
			<input type="checkbox" name="<?=$res["NAME"]?>" id="<?=$res["ID"]?>" value="<?=$res["VALUE"]?>" class="input<?=$res["CLASS"]?>" <?
				?><?=($res["ACTIVE"] == $res["VALUE"] ? " checked='checked' " : "")?> />
			<label class="label" for="<?=$res["ID"]?>"><?=$res["LABEL"]?></label>
		</div>
<?
		else:
			if (!empty($_REQUEST["del_filter"]))
...
?>
		<span class="forum-filter-first">
				<div class="btn ok2 btn-green mode2 icon-arrow-right" onclick="this.firstElementChild.click();">		
					<input class="ok-btn2" type="submit" name="set_filter" value="<?=GetMessage("FORUM_BUTTON_FILTER")?>" /></span>
				</div>
	
				<div class="btn ok2 btn-gray-dark mode2 icon-arrow-right" onclick="this.firstElementChild.click();">
					<input class="ok-btn2" type="submit" name="del_filter" value="<?=GetMessage("FORUM_BUTTON_RESET")?>" />
				</div>
		</span>
<?
	else:
...
?>
		<span class="<?=($counter == 0 ? "forum-filter-first" : "")?>">
			<div class="btn ok2 btn-gray-dark mode2 icon-arrow-right" onclick="this.firstElementChild.click();">
				<input class="ok-btn2" type="submit" name="<?=$res["NAME"]?>" value="<?=$res["VALUE"]?>" />
			</div>
		</span>
<?
		endforeach;
