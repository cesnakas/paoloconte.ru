...
$imageConfig = array(
	...
	'SIZE' => array(
		'SMALL' => array('W'=>56,'H'=>100),
        'BIG' => array('W'=>360,'H'=>480),
...
if(!empty($modelsGroup)) {
	...
	$arFilter = array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "PROPERTY_GRUPPIROVKA_PO_KOLODKE_SAYT" => $modelsGroup, "ACTIVE" => "Y");
...
$res = CIBlockElement::GetList(array('ID' => 'ASC'), $arFilter, false, false, array("ID", "IBLOCK_ID", "NAME", "DATE_CREATE", "PROPERTY_USER_NAME", "PROPERTY_MESSAGE"));
while ($ob = $res->GetNext(false, false)) {
	$arResult['REVIEWS'][] = $ob;
}
...
if(!empty($arResult['OFFERS'])) {
	...
	foreach ($arResult['OFFERS'] as $key => $arOffer) {
		if ($arOffer['PRICES'][$price_name]['DISCOUNT_VALUE'] < $oldPrice) {
			$oldPrice = $arOffer['PRICES'][$price_name]['DISCOUNT_VALUE'];
			$oldPrice_print = $arOffer['PRICES'][$price_name]['PRINT_DISCOUNT_VALUE'];
		}
		if ($arOffer['PRICES'][$price_action_name]['DISCOUNT_VALUE'] < $newPrice) {
...
// МАГАЗИНЫ И НАЛИЧИЕ
$arShops = \Citfact\Paolo::GetShopsByFilter(array('PROPERTY_CITY' => $_SESSION['CITY_ID']));
// Собираем id складов
$arIdsStores = array();
...
$arAmount = array();
if (!empty($arIdsOffers) && !empty($arIdsStores)) {
    $rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => array_merge($arIdsOffers, $deactivatedOffersIds), 'STORE_ID' => $arIdsStores), false, false, array('PRODUCT_ID', 'STORE_ID'));
...
// Смотрим наличие торговых предложений в складе «Интернет-магазин» и склад "Sale"
