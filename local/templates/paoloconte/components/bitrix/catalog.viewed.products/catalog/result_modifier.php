...
// Убираем товары без цены текущего региона
$str_price = 'CATALOG_PRICE_'.$_SESSION['GEO_PRICES']['PRICE_ID'];
foreach ($arResult['ITEMS'] as $key=>$arItem) {
	if ($arItem[$str_price] <= 0) {
		unset($arResult['ITEMS'][$key]);
	}
}

/*ПОДГОТОВКА КАРТИНОК*/
$imageConfig = array('TYPE'=>'ONE', 'SIZE' => array('SMALL' => array('W'=>150,'H'=>200)));
foreach ($arResult['ITEMS'] as $key => $arItem){
...
foreach ($arResult['ITEMS'] as $key=>$arItem) {
	...
	if(!empty($arItem['OFFERS'])) {
		...
		foreach ($arItem['OFFERS'] as $arOffer) {
			...
			if ($arOffer['PRICES'][$price_action_name]['VALUE'] < $newPrice) {
				$newPrice = $arOffer['PRICES'][$price_action_name]['VALUE'];
				$newPrice_print = $arOffer['PRICES'][$price_action_name]['PRINT_VALUE'];
			}
		}
		$arResult['ITEMS'][$key]['OLD_PRICE'] = number_format($oldPrice, 0, ',', ' ');
		$arResult['ITEMS'][$key]['NEW_PRICE'] = number_format($newPrice, 0, ',', ' ');;
	}
	else{
		$arResult['ITEMS'][$key]['OLD_PRICE'] = number_format($arItem['CATALOG_PRICE_'.$price_id], 0, ',', ' ');
		$arResult['ITEMS'][$key]['NEW_PRICE'] = number_format($arItem['CATALOG_PRICE_'.$price_action_id], 0, ',', ' ');
	}
}
?>