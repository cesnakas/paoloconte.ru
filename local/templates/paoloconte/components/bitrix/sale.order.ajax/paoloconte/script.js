BX.saleOrderAjax = {

	BXCallAllowed: false,

	options: {},
	indexCache: {},
	controls: {},

	modes: {},
	properties: {},

	// called once, on component load
	init: function(options)
	{
		var ctx = this;
		this.options = options;

		window.submitFormProxy = BX.proxy(function(){
			ctx.submitFormProxy.apply(ctx, arguments);
		}, this);

		BX(function(){
			ctx.initDeferredControl();
		});
		BX(function(){
			ctx.BXCallAllowed = true; // unlock form refresher
		});

		this.controls.scope = BX('order_form_div');

		// user presses "add location" when he cannot find location in popup mode
		BX.bindDelegate(this.controls.scope, 'click', {className: '-bx-popup-set-mode-add-loc'}, function(){

			var input = BX.create('input', {
				attrs: {
					type: 'hidden',
					name: 'PERMANENT_MODE_STEPS',
					value: '1'
				}
			});

			BX.prepend(input, BX('ORDER_FORM'));

			ctx.BXCallAllowed = false;
			submitForm();
		});
	},

	cleanUp: function(){

		for(var k in this.properties)
		{
			if (this.properties.hasOwnProperty(k))
			{
				if(typeof this.properties[k].input != 'undefined')
				{
					BX.unbindAll(this.properties[k].input);
					this.properties[k].input = null;
				}

				if(typeof this.properties[k].control != 'undefined')
					BX.unbindAll(this.properties[k].control);
			}
		}

		this.properties = {};
	},

	addPropertyDesc: function(desc){
		this.properties[desc.id] = desc.attributes;
		this.properties[desc.id].id = desc.id;
	},

	// called each time form refreshes
	initDeferredControl: function()
	{
		console.debug('Init deferred control');

		var ctx = this,
			k,
			row,
			input,
			locPropId,
			m,
			control,
			code,
			townInputFlag,
			adapter;

		// first, init all controls
		if(typeof window.BX.locationsDeferred != 'undefined'){

			this.BXCallAllowed = false;

			for(k in window.BX.locationsDeferred){

				window.BX.locationsDeferred[k].call(this);
				window.BX.locationsDeferred[k] = null;
				delete(window.BX.locationsDeferred[k]);

				this.properties[k].control = window.BX.locationSelectors[k];
				delete(window.BX.locationSelectors[k]);
			}
		}

		for(k in this.properties){

			// zip input handling
			if(this.properties[k].isZip){
				row = this.controls.scope.querySelector('[data-property-id-row="'+k+'"]');
				if(BX.type.isElementNode(row)){

					input = row.querySelector('input[type="text"]');
					if(BX.type.isElementNode(input)){
						this.properties[k].input = input;

						// set value for the first "location" property met
						locPropId = false;
						for(m in this.properties){
							if(this.properties[m].type == 'LOCATION'){
								locPropId = m;
								break;
							}
						}

						if(locPropId !== false){
							BX.bindDebouncedChange(input, function(value){

								input = null;
								row = null;

								if(BX.type.isNotEmptyString(value) && /^\s*\d+\s*$/.test(value) && value.length > 3){

									ctx.getLocationByZip(value, function(locationId){
										ctx.properties[locPropId].control.setValueByLocationId(locationId);
									}, function(){
										try{
											ctx.properties[locPropId].control.clearSelected(locationId);
										}catch(e){}
									});
								}
							});
						}
					}
				}
			}

			if(this.properties[k].type == 'LOCATION'){

				if(typeof this.properties[k].control != 'undefined'){

					control = this.properties[k].control;
					code = control.getSysCode();

					// replace default boring "nothing found" label for popup with "-bx-popup-set-mode-add-loc" inside
					if(code == 'sls') // for sale.location.selector.search
						control.replaceTemplate('nothing-found', this.options.messages.notFoundPrompt);

					// handle town input field
					if(code == 'slst'){  // for sale.location.selector.steps
						if(typeof this.properties[k].altLocationPropId != 'undefined'){

							// control can have "select other location" option
							control.setOption('pseudoValues', ['other']);

							// when control tries to search for items
							control.bindEvent('before-control-item-discover-done', function(knownItems, adapter){

								control = null;

								var parentValue = adapter.getParentValue();

								// you can choose "other" location only if parentNode is not root and is selectable
								if(parentValue == this.getOption('rootNodeValue') || !this.checkCanSelectItem(parentValue))
									return;

								knownItems.unshift({DISPLAY: ctx.options.messages.otherLocation, VALUE: 'other', CODE: 'other', IS_PARENT: false});
							});

							(function(k, control){

								control.bindEvent('after-select-real-value', function(){
									var townInputFlag = BX('LOCATION_ALT_PROP_DISPLAY_MANUAL['+parseInt(k)+']');
									if(BX.type.isDomNode(townInputFlag))
										townInputFlag.value = '0';
								});
								control.bindEvent('after-select-pseudo-value', function(){
									var townInputFlag = BX('LOCATION_ALT_PROP_DISPLAY_MANUAL['+parseInt(k)+']');
									if(BX.type.isDomNode(townInputFlag))
										townInputFlag.value = '1';
								});

							})(k, control);

							// restore "other location" label on the last control

							townInputFlag = BX('LOCATION_ALT_PROP_DISPLAY_MANUAL['+parseInt(k)+']');
							if(BX.type.isDomNode(townInputFlag) && townInputFlag.value == '1'){

								// a little hack: set "other location" text display
								adapter = control.getAdapterAtPosition(control.getStackSize() - 1);

								if(typeof adapter != 'undefined' && adapter !== null)
									adapter.setValuePair('other', ctx.options.messages.otherLocation);
							}
						}
					}
				}
			}
		}

        // init masked inputs
		if (typeof Application.Components.DopComponents != 'undefined') {
            Application.Components.DopComponents.setMaskedInputs();
        }

		this.BXCallAllowed = true;
	},

	checkMode: function(propId, mode){

		//if(typeof this.modes[propId] == 'undefined')
		//	this.modes[propId] = {};

		//if(typeof this.modes[propId] != 'undefined' && this.modes[propId][mode])
		//	return true;

		if(mode == 'altLocationChoosen'){

			if(this.checkAbility(propId, 'canHaveAltLocation')){

				var input = this.getInputByPropId(this.properties[propId].altLocationPropId);
				var altPropId = this.properties[propId].altLocationPropId;

				if(input !== false && input.value.length > 0 && !input.disabled && this.properties[altPropId].valueSource != 'default'){

					//this.modes[propId][mode] = true;
					return true;
				}
			}
		}

		return false;
	},

	checkAbility: function(propId, ability){

		if(typeof this.properties[propId] == 'undefined')
			this.properties[propId] = {};

		if(typeof this.properties[propId].abilities == 'undefined')
			this.properties[propId].abilities = {};

		if(typeof this.properties[propId].abilities != 'undefined' && this.properties[propId].abilities[ability])
			return true;

		if(ability == 'canHaveAltLocation'){

			if(this.properties[propId].type == 'LOCATION'){

				// try to find corresponding alternate location prop
				if(typeof this.properties[propId].altLocationPropId != 'undefined' && typeof this.properties[this.properties[propId].altLocationPropId]){

					var altLocPropId = this.properties[propId].altLocationPropId;

					if(typeof this.properties[propId].control != 'undefined' && this.properties[propId].control.getSysCode() == 'slst'){

						if(this.getInputByPropId(altLocPropId) !== false){
							this.properties[propId].abilities[ability] = true;
							return true;
						}
					}
				}
			}

		}

		return false;
	},

	getInputByPropId: function(propId){
		if(typeof this.properties[propId].input != 'undefined')
			return this.properties[propId].input;

		var row = this.getRowByPropId(propId);
		if(BX.type.isElementNode(row)){
			var input = row.querySelector('input[type="text"]');
			if(BX.type.isElementNode(input)){
				this.properties[propId].input = input;
				return input;
			}
		}

		return false;
	},

	getRowByPropId: function(propId){

		if(typeof this.properties[propId].row != 'undefined')
			return this.properties[propId].row;

		var row = this.controls.scope.querySelector('[data-property-id-row="'+propId+'"]');
		if(BX.type.isElementNode(row)){
			this.properties[propId].row = row;
			return row;
		}

		return false;
	},

	getAltLocPropByRealLocProp: function(propId){
		if(typeof this.properties[propId].altLocationPropId != 'undefined')
			return this.properties[this.properties[propId].altLocationPropId];

		return false;
	},

	toggleProperty: function(propId, way, dontModifyRow){

		var prop = this.properties[propId];

		if(typeof prop.row == 'undefined')
			prop.row = this.getRowByPropId(propId);

		if(typeof prop.input == 'undefined')
			prop.input = this.getInputByPropId(propId);

		if(!way){
			if(!dontModifyRow)
				BX.hide(prop.row);
			prop.input.disabled = true;
		}else{
			if(!dontModifyRow)
				BX.show(prop.row);
			prop.input.disabled = false;
		}
	},

	submitFormProxy: function(item, control)
	{
		var propId = false;
		for(var k in this.properties){
			if(typeof this.properties[k].control != 'undefined' && this.properties[k].control == control){
				propId = k;
				break;
			}
		}

		// turning LOCATION_ALT_PROP_DISPLAY_MANUAL on\off

		if(item != 'other'){

			if(this.BXCallAllowed){

				this.BXCallAllowed = false;
				submitForm();
			}

		}
	},

	getPreviousAdapterSelectedNode: function(control, adapter){

		var index = adapter.getIndex();
		var prevAdapter = control.getAdapterAtPosition(index - 1);

		if(typeof prevAdapter !== 'undefined' && prevAdapter != null){
			var prevValue = prevAdapter.getControl().getValue();

			if(typeof prevValue != 'undefined'){
				var node = control.getNodeByValue(prevValue);

				if(typeof node != 'undefined')
					return node;

				return false;
			}
		}

		return false;
	},
	getLocationByZip: function(value, successCallback, notFoundCallback)
	{
		if(typeof this.indexCache[value] != 'undefined')
		{
			successCallback.apply(this, [this.indexCache[value]]);
			return;
		}

		ShowWaitWindow();

		var ctx = this;

		BX.ajax({

			url: this.options.source,
			method: 'post',
			dataType: 'json',
			async: true,
			processData: true,
			emulateOnload: true,
			start: true,
			data: {'ACT': 'GET_LOC_BY_ZIP', 'ZIP': value},
			//cache: true,
			onsuccess: function(result){

				CloseWaitWindow();
				if(result.result){

					ctx.indexCache[value] = result.data.ID;

					successCallback.apply(ctx, [result.data.ID]);

				}else
					notFoundCallback.call(ctx);

			},
			onfailure: function(type, e){

				CloseWaitWindow();
				// on error do nothing
			}

		});
	}

}

function checkUserCloudLoyalty(orderClick = false){
    var cBox = "false";
    if (typeof($("#need_bonus_false")[0]) !== "undefined")
    {
        cBox = $("#need_bonus_false")[0].checked;
    }
	$('[data-ajax-error]').remove();
    var userCard = $('[data-card]').children('input').val();
    if (userCard.length == 0){
        userCard = $('[data-card]').children('input').attr('value');
    }
	var ajax = BX.ajax({
		url: BX.message('TEMPLATE_PATH_CITFACT_CLOUDLOYALTY_AJAX')+'/ajaxCloudloyalty.php',
		data: {'PHONE':$('[data-phone]').children('input').val(), 'CARD': userCard, 'ENABLE': cBox, 'ORDERCLICK':orderClick},
		method: 'POST',
		dataType: 'html',
		async: false,
		timeout: 60,
		processData: true,
		scriptsRunFirst: true,
		emulateOnload: true,
		cache: false,
		onsuccess: function(data){
			$('[data-card]').parent().append(data);
		},
		onfailure: function(){

		}
	});

	//если пришел не пустой ответ, то отменяем отправку формы
  
	if(ajax.response.length > 0){
        if (ajax.response == "Y" || ajax.response == "N"){
            document.location.reload();
        }
        else if (
            ajax.response == '<span style="display:none">disable_phone</span>' ||
            ajax.response == '<span style="display:none">disable_card</span>' ||
            ajax.response.indexOf('order-errors-cont') > 0
            )
        {
            $("#need_bonus_false").attr("disabled");
            $("#need_bonus_false").addClass("disabled");
            $("#need_bonus_false").parent('.b-checkbox__label').addClass("disabled");

        }
		return false;
	}
	return true;
}

function updateBasketUserCloudLoyalty(){
	var phone = $('[data-phone]').children('input').val();
	var card = $('[data-card]').children('input').val();
	if(card.length > 0){
		checkUserCloudLoyalty();
	}
	if (phone.length > 0  && card.length > 0) {
		BX.ajax({
			url: BX.message('TEMPLATE_PATH_CITFACT_CLOUDLOYALTY_AJAX') + '/ajaxUpdateBonusCloudloyality.php',
			data: {},
			method: 'POST',
			dataType: 'json',
			async: true,
			timeout: 60,
			processData: true,
			scriptsRunFirst: true,
			emulateOnload: true,

			cache: false,
			onsuccess: function (data) {
				$('[data-basket-order]').empty();
				$('[data-basket-order]').append(data.basket);
				$('[data-coment-order]').empty();
				$('[data-coment-order]').append(data.comment);
				$('[data-onload-content]').removeClass('hidden');

			},
		});
		submitForm();
	}
}

$(document).ready(function () {

    if (typeof submitForm !== "undefined") {
        submitForm();
    }

    $('body').on('change', 'select[name="ADDRESS_MY"]', function () {
        var val = $(this).val();
        if (val == 'new') {
            $('.my-addresses').removeClass('hidden');
        } else {
            $('input.location-id').val($('#address_' + val).data('location-id'));
            submitForm();
        }
    });
    $(document).on('change', '[data-card]', function () {
        updateBasketUserCloudLoyalty();
    });
    $(document).on('change', '.mask-phone', function () {
        updateBasketUserCloudLoyalty();
    });

    $('body').on('click', '#sheepla-widget-control input[type="button"]', function () {
        var pos = $('#sheepla-sdk-lock-logo').offset();
        $('html,body').animate({scrollTop: pos.top - 10}, 0);
    });

    selfDeliveryManager.check();
});

function getErrorDiv(text) {
    return '<div class="order-errors-cont show" style="padding: 0px;">\n' +
        '\n<p><font class="errortext">' + text + '</font></p></div>'
}

function validateForm() {
    var isValid = true;
	var phoneInput = $('.mask-phone');
	var surnameInput = $('[data-prop-code="SURNAME"]').find('input');
	var nameInput = $('[data-prop-code="NAME"]').find('input');
	var secondInput = $('[data-prop-code="SECOND_NAME"]').find('input');
	var emailInput = $('[data-prop-code="EMAIL"]').find('input');
	var streetInput = $('input.property-STREET');
	var houseInput = $('input.property-HOUSE');

    if (!selfDeliveryManager.validate()) {
        window.scrollTo($('.basket-item__not-avail-delivery.show')[0]);
        isValid = false;
        return false;
    }

    $('.order-errors-cont').each(function (i, error) {
        error.remove();
        if (surnameInput.hasClass('input-error')) {
			surnameInput.removeClass('input-error');
		}
		if (nameInput.hasClass('input-error')) {
			nameInput.removeClass('input-error');
		}
		if (secondInput.hasClass('input-error')) {
			secondInput.removeClass('input-error');
		}
		if (emailInput.hasClass('input-error')) {
			emailInput.removeClass('input-error');
		}
		if (streetInput.hasClass('input-error')) {
			streetInput.removeClass('input-error');
		}
		if (houseInput.hasClass('input-error')) {
			houseInput.removeClass('input-error');
		}
    });

    var noDeliveries = $('#ORDER_FORM #no_deliveries');
    if (noDeliveries.length > 0) {
        window.scrollTo(noDeliveries[0]);
        return false;
    }

    if (phoneInput.val() === '') {
        isValid = false;
        phoneInput.after(getErrorDiv('* Это поле обязательно'));
    }

	if (surnameInput.length > 0) {
		surnameInput.val(surnameInput.val().trim());
		if (surnameInput.val() === '') {
			isValid = false;
			surnameInput.after(getErrorDiv('* Это поле обязательно'));
			surnameInput.addClass('input-error');
		} else if(surnameInput.val().match(/[^А-Яа-яЁёA-Za-z-]/g) !== null) {
			isValid = false;
			surnameInput.after(getErrorDiv('* Это поле не может содержать пробелы и специальные символы'));
			surnameInput.addClass('input-error');
		}
	}

	if (nameInput.length > 0) {
		nameInput.val(nameInput.val().trim());
		if (nameInput.val() === '') {
			isValid = false;
			nameInput.after(getErrorDiv('* Это поле обязательно'));
			nameInput.addClass('input-error');
		} else if(nameInput.val().match(/[^А-Яа-яЁёA-Za-z-]/g) !== null) {
			isValid = false;
			nameInput.after(getErrorDiv('* Это поле не может содержать пробелы и специальные символы'));
			nameInput.addClass('input-error');
		}
	}

	if (secondInput.length > 0) {
		secondInput.val(secondInput.val().trim());
		if(secondInput.val().match(/[^А-Яа-яЁёA-Za-z-]/g) !== null) {
			isValid = false;
			secondInput.after(getErrorDiv('* Это поле не может содержать пробелы и специальные символы'));
			secondInput.addClass('input-error');
		}
	}

	setFullName();
    var fioInput = $('[data-prop-code="FIO"]').find('input');
    fioInput.val(fioInput.val().trim());
    if (fioInput.val() === '') {
        isValid = false;
        fioInput.after(getErrorDiv('* Необходимо ввести имя и фамилия'));
    }

    if (emailInput.val() === '') {
        isValid = false;
        emailInput.after(getErrorDiv('* Необходимо ввести E-mail'));
		emailInput.addClass('input-error');
    }

    if (streetInput.length > 0 && streetInput.val() === '') {
        isValid = false;
		streetInput.after(getErrorDiv('* Необходимо ввести улицу'));
		streetInput.addClass('input-error');
    }

    if (houseInput.length > 0 && houseInput.val() === '') {
        isValid = false;
		houseInput.after(getErrorDiv('* Необходимо ввести дом'));
		houseInput.addClass('input-error');
    }

    if ($('#ID_DELIVERY_54').attr("checked") == 'checked' && $('.basket-item').length) {
        BX.ajax({
            url: BX.message('TEMPLATE_PATH_CITFACT_CLOUDLOYALTY_AJAX') + '/ajaxAvailabilityGoods.php',
            data: {'STORE': $("#delivery_store :selected").attr("data-id")},
            method: 'POST',
            dataType: 'json',
            async: false,
            timeout: 10,
            processData: true,
            onsuccess: function (data) {
                if (data.result == false) {
                    isValid = false;
                    if (data.message) {
                        $('#delivery-params').before(getErrorDiv(data.message));
                    }
                    if (data.ids) {
                        var visibleError;
                        $.each(data.ids, function (id, val) {
                            visibleError = val ? 'none' : '';
                            $('[data-product-id="' + id + '"]').find(".basket-item__not-avail-delivery").css("display", visibleError);
                        });
                    }
                } else {
                    $('.basket-item').find(".basket-item__not-avail-delivery").css("display", "none");
                }
            },
        });
    } else {
        $('.basket-item').find(".basket-item__not-avail-delivery").css("display", "none");
    }

    if (!isValid) {
		var errors = $('#sale_order_props .order-errors-cont.show');
		if (errors.length > 0) {
			window.scrollTo($('#sale_order_props')[0]);
		} else {
			window.scrollTo($('.order-form__address')[0]);
		}
    }

    return isValid
}

$(document).on('input', '[data-prop-code="FIO"] input', function () {
    this.value = this.value.replace(/[^А-Яа-яЁёA-Za-z ]/g, "");
});

$(document).on('input', 'input.property-SURNAME, input.property-NAME, input.property-SECOND_NAME', function () {
	// this.value = this.value.replace(/[^А-Яа-яЁёA-Za-z-]/g, "");
});

var selfDeliveryManager = {
	arrStoryCheck: [],
	arrResponseStoryCheck: {},
	isSending: false,
	isInitDelivery: false,
	isStoryCheck(storeId) {
		if (this.arrStoryCheck.indexOf(storeId) === -1) {
			return false;
		}
		return true;
	},
    check() {
        var self = this;
        if(self.isSending) {
        	return;
		}
        this.clearNotAvailProduct();
        var input = $('#ID_DELIVERY_' + window.idMySelfDelivery);
        if (input.length > 0 && input.prop('checked') === true) {
            var select = input.parent().find('select#delivery_store');
            if (select.length > 0) {
                var products = [];
                $('input.quantityInputs').each(function (i, v) {
                    products.push($(v).attr('data-product-id'));
                });
                var storeId = select.find('option:selected').attr('data-id');
				if (!self.isStoryCheck(storeId)) {
					BX.showWait();
					self.isSending = true;
					$.ajax({
						url: '/include/ajax_get_availability_count_product_retail.php',
						data: {
							idShop: storeId,
							products: products
						},
						dataType: "json",
						async: true,
						success: function (data) {
							self.arrStoryCheck.push(storeId);
							self.arrResponseStoryCheck[storeId] = data;
							self.success(data);
							self.isSending = false;
							BX.closeWait();
						},
						error: function () {
							self.isSending = false;
							BX.closeWait();
						}
					});
				} else {
					self.success(self.arrResponseStoryCheck[storeId]);
				}
            }
        }
    },

	success(data) {
		var self = this;
		$.each(data, function (idProduct, realCount) {
			var inputCount = $('.quantityInputs[data-product-id="' + idProduct + '"]');
			var setCount = inputCount.val();
			if (setCount > realCount) {
				self.addNotAvailProduct(inputCount);
			}
		});
	},

    clearNotAvailProduct() {
        var notAvailProductMessages = $('.basket-item__not-avail-delivery');
        notAvailProductMessages.removeClass('show');
    },

    addNotAvailProduct(inputCount) {
        var notAvailProductMessages = inputCount.closest('.basket-item').find('.basket-item__not-avail-delivery');
        notAvailProductMessages.addClass('show');
    },

    validate() {
        var notAvailProductMessagesActive = $('.basket-item__not-avail-delivery.show');
        if (notAvailProductMessagesActive.length > 0) {
            return false;
        }
        return true;
    },

	initDelivery() {
		if (!this.isInitDelivery) {
			if ($('#delivery-params input[name="DELIVERY_ID"]:checked').length <= 0) {
				var inputDelivery = $('#delivery-params input[name="DELIVERY_ID"]');
				if (inputDelivery.length > 0) {
					inputDelivery[0].click();
				}
			}
			this.isInitDelivery = true;
		}
	}
};