...
/*ПОДГОТОВКА КАРТИНОК*/
$imageConfig = array('TYPE'=>'ONE', 'SIZE' => array('SMALL' => array('W'=>400,'H'=>400)));
foreach ($arResult['ITEMS'] as $key => $arItem){
...
foreach ($arResult['ITEMS'] as $key=>$arItem) {
	...
	if(!empty($arItem['OFFERS'])) {
		...
		$arResult['ITEMS'][$key]['OLD_PRICE'] = $oldPrice_print;
		$arResult['ITEMS'][$key]['NEW_PRICE'] = $newPrice_print;
	}
	else{
		$arResult['ITEMS'][$key]['OLD_PRICE'] = CurrencyFormat($arItem['CATALOG_PRICE_'.$price_id],'RUB');
		$arResult['ITEMS'][$key]['NEW_PRICE'] = CurrencyFormat($arItem['CATALOG_PRICE_'.$price_action_id],'RUB');
	}
}
