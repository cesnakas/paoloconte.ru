...
//              $order->save();
              ...
              // Прописываем дополнительные свойства заказа при ответе из cloudPayments
              $this->setAdditionalOrderProps($payment, $request);
...
//                    endif;
    ...
    /**
     * Прописываем дополнительные свойства заказа при различных статусах.
     *
     * @param Payment $payment
     * @param Request $request
     */
    private function setAdditionalOrderProps(Payment $payment, Request $request)
    {
        $paymentFilelds = $payment->getFieldValues();      // поля оплаты
        $requestgetPostList = $request->getPostList();     // получение списка POST-параметров
        $requestgetQueryList = $request->getQueryList();   // получение списка GET-параметров
        ...
        $PROPERTY_PS_ORDER_ID = 44; // id свойства заказа "Платежная система: id заказа" - "PS_ORDER_NUMBER" (в сбербанке тоже оно)
        $orderID = $requestgetPostList['InvoiceId'];
        $action = $requestgetQueryList['action'];

        // При подтверждении оплаты cloudPayments стучится на сайт с этим статусом.
        if ($action == 'confirm' || $action == 'pay') {
            $arOrderFields = array(
                "PS_SUM" => $paymentFilelds['SUM'],
                "PS_CURRENCY" => $arDefaultIso[$requestgetPostList['Currency']],
                "PS_RESPONSE_DATE" => Date(\CDatabase::DateFormatToPHP(\CLang::GetDateFormat("FULL", LANG))),
                "PS_STATUS" => "Y",
                "PS_STATUS_DESCRIPTION" => $requestgetPostList['CardFirstSix'] . 'XXXXXX'. $requestgetPostList['CardLastFour'] /*. ";" . $requestgetPostList['Name']*/,
                "PS_STATUS_MESSAGE" => $requestgetPostList['Status'],
                "PS_STATUS_CODE" => "Y",
            );
            ...
            $arParameters = [
                ...
                'VALUE' => $requestgetPostList['TransactionId'] // id транзакции из cloudPayments
            ];

            $dbP = \CSaleOrderPropsValue::GetList([], ['ORDER_ID' => $orderID, 'ORDER_PROPS_ID' => $PROPERTY_PS_ORDER_ID]);
            if ($arP = $dbP->Fetch()) {
                \CSaleOrderPropsValue::Update($arP['ID'], $arParameters);
            } else {
                \CSaleOrderPropsValue::Add($arParameters);
            }

            \CSaleOrder::Update($orderID, $arOrderFields);
        } // можно добавить обработку остальных action -- check, pay (отрабатывают при отправке заказа в cloudpayments)..
    }
