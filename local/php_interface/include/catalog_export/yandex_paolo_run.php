...
$imageConfig = array('TYPE' => 'ONE', 'SIZE' => array('SMALL' => array('W' => 500, 'H' => 665)));
        ...
        if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
        {
            ...
            switch ($PROPERTY_CODE) {
                case 'TSVET_MARKETING':
                    $arProperty['NAME'] = $APPLICATION->ConvertCharset(Loc::getMessage('PROPERTY_TSVET_MARKETING'), 'windows-1251', LANG_CHARSET);
                    break;
            ...
            if ($bParam)
            {
                if (is_array($description))
                {
                    foreach ($value as $key => $val)
                    {
                        ...
                        $strProperty .= '<param name="'.yandex_text2xml($description[$key], true).'" '.$addParams.' >'.
                            yandex_text2xml($val, true).'</param>';
                    }
                }
                else
                {
                    $strProperty .= '<param name="'.yandex_text2xml($arProperty['NAME'], true).'" '.$addParams.' >'.
...
if (empty($arRunErrors))
{
    ...
    else
    {
        ...
        else
        {
            if (!$disableReferers)
            {
                fwrite($fp, 'if (!isset($_GET["referer1"]) || strlen($_GET["referer1"])<=0) $_GET["referer1"] = "yandexm";'."\n");
                fwrite($fp, 'if (!isset($_GET["referer1"]) || strlen($_GET["referer1"])<=0) $_GET["referer1"] = "yandexm";'."\n");
    ...
    elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
    {
        ...
        while ($obItem = $rsItems->GetNextElement())
        {
            ...
            foreach ($arItem['PROPERTIES'] as $itemProperty) {
                if ($itemProperty['CODE'] == 'CML2_ARTICLE') {
                    $itemArticle = $itemProperty['VALUE'];
                    $arItem['GET_ARTICUL_IMAGE'] = Citfact\Paolo::getProductImage($itemArticle, $imageConfig);
                    if (!empty($arItem['GET_ARTICUL_IMAGE']['PHOTO'][0]['SMALL'])) {
                        $strFile = $usedProtocol.$ar_iblock['SERVER_NAME'].CHTTP::urnEncode($arItem['GET_ARTICUL_IMAGE']['PHOTO'][0]['SMALL'], 'utf-8');
                    }
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
                    {
                        ...
                        foreach ($arCurrentOffer['PROPERTIES'] as $ofProperty) {
                            if ($ofProperty['CODE'] == 'CML2_ARTICLE') {
                                ...
                                $articleImage =  Citfact\Paolo::getProductImage($ofArticle, $imageConfig);
                                if (!empty($articleImage['PHOTO'][0]['SMALL'])) {
                                    $strFile = $usedProtocol.$ar_iblock['SERVER_NAME'].CHTTP::urnEncode($articleImage['PHOTO'][0]['SMALL'], 'utf-8');
                                }
                        ...
                        if (!empty($strFile)) {
                            ...
                            if (!empty($strFile) || !empty($arItem['YANDEX_PICT']))
                            {
                                $strOfferYandex .= "<picture>".$strFile."</picture>\n";
                            }
                ...
                else
                {
                    while ($obOfferItem = $rsOfferItems->GetNextElement())
                    {
                        ...
                        foreach ($arOfferItem['PROPERTIES'] as $ofProperty) {
                            if ($ofProperty['CODE'] == 'CML2_ARTICLE') {
                                $ofArticle = $ofProperty['VALUE'];
                                $arOfferItem['GET_ARTICUL_IMAGE'] = Citfact\Paolo::getProductImage($ofArticle, $imageConfig);
                                if (!empty($arOfferItem['GET_ARTICUL_IMAGE']['PHOTO'][0]['SMALL'])) {
                                    $strFile = $usedProtocol.$ar_iblock['SERVER_NAME'].CHTTP::urnEncode($arOfferItem['GET_ARTICUL_IMAGE']['PHOTO'][0]['SMALL'], 'utf-8');
                                }
                            } elseif ($ofProperty['CODE'] == 'NAIMENOVANIE_MARKETING' && !empty($ofProperty['VALUE'])) {
            ...
            elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL && $arItem['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_PRODUCT) {
                ...
                if (!empty($arItem['YANDEX_PICT'])) {
                    ...
                    $strOfferYandex .= "<url>" . htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]) . $referer . "</url>\n";
                    ...
                    foreach ($arYandexFields as $key) {
                        ...
                        switch ($key) {
                            ...
                            case 'description':
                                ...
                                if (empty($text))
                                    $text = Loc::getMessage('YANDEX_CUSTOM_DESCRIPTION');
                                $strValue = "<description>" . $text . "</description>\n";
                                break;
                            case 'vendor':
                                $strValue .= "<vendor>" . Loc::getMessage('YANDEX_VENDOR_PAOLO_CONTE') . "</vendor>\n";
                                break;
                            case 'vendorCode':
                                $strValue .= "<vendorCode>" . $itemArticle . "</vendorCode>\n";
                                break;
                            case 'param':
                                if ($parametricFieldsExist) {
                                    foreach ($parametricFields as $paramKey => $prop_id) {
            ...
            $arItem['count_offers'] = count($offers);

            if ($arItem['count_offers'] >= \Citfact\Core::getInstance()->getExportMinCountOfferForSend()) {
